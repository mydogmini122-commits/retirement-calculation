...
export const getFinancialAdvice = async (
  ...
  // 核心修正：使用最直接的讀取方式，確保能抓到 Vite 注入的變數
  const apiKey = (globalThis as any).__GEMINI_API_KEY__ || process.env.GEMINI_API_KEY;

  if (!apiKey) {
    throw new Error("API Key 未能成功注入，請檢查 Vercel 設定並 Redeploy");
  }
